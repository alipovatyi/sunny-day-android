...
private fun LocationDetailsScreen(
    ...
    val locationWithForecasts = viewState.locationWithForecasts
    ...
    Scaffold(
            ...
            CenterAlignedTopAppBar(
                title = {
                    Row(verticalAlignment = Alignment.CenterVertically) {
                        Text(text = locationWithForecasts?.name.orEmpty())
                        if (locationWithForecasts?.isCurrent == true) {
                ...
                actions = {
                    if (locationWithForecasts?.isCurrent == false) {
        ...
        Column(
            ...
            if (locationWithForecasts != null) {
                val currentDailyForecast = locationWithForecasts.dailyForecasts.first()
                // TODO: move filtering to view model
                val hourlyForecasts = remember(locationWithForecasts.hourlyForecasts) {
                    locationWithForecasts.hourlyForecasts.filter { hourlyForecast ->
                        val now = LocalDateTime.now()
                        hourlyForecast.time in now..now.plusDays(1)
                    }
                }
                val dailyForecasts = remember(locationWithForecasts.dailyForecasts) {
                    locationWithForecasts.dailyForecasts.filter { dailyForecast ->
                        dailyForecast.time >= LocalDateTime.now().toLocalDate()
                    }
                }
                ...
                AirConditionsSection(
                    ...
                    currentHourlyForecast = hourlyForecasts.first(),
                    modifier = Modifier.fillMaxWidth()
                )
...
private fun LocationDetailsScreenPreview() {
    SunnyDayThemePreview {
        LocationDetailsScreen(
            viewState = LocationDetailsViewState(
                locationWithForecasts = LocationWithForecasts(
